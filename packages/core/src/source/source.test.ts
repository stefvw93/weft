    ...
    it("get succeeds with the static value", async () => {
      const value = await scoped(
        Effect.gen(function* () {
          ...
          return yield* Subscribable.get(sub);
        }),
      );
    ...
    it("changes emits the value exactly once then completes", async () => {
      const values = await scoped(
        Effect.gen(function* () {
          ...
          return yield* pipe(Subscribable.changes(sub), Stream.runCollect);
        }),
      );
      ...
      const exit = await Effect.runPromise(
        Effect.scoped(
          Effect.gen(function* () {
            ...
            return yield* Effect.exit(Subscribable.get(sub));
          }),
        ),
      );
    ...
    it("underlying effect runs exactly once across multiple get/changes consumers", async () => {
      ...
      await scoped(
        Effect.gen(function* () {
          ...
          const v1 = yield* Subscribable.get(sub);
          const v2 = yield* Subscribable.get(sub);
          const items = yield* pipe(Subscribable.changes(sub), Stream.runCollect);
    ...
    it("get returns the most recent emitted value without re-awaiting", async () => {
      ...
      const value = await scoped(
        Effect.gen(function* () {
          ...
          yield* pipe(Subscribable.changes(sub), Stream.take(1), Stream.runDrain);
          return yield* Subscribable.get(sub);
        }),
      );
    ...
    it("get parks and resolves with the first emitted value", async () => {
      ...
      const value = await scoped(
        Effect.gen(function* () {
          ...
          const getFiber = yield* Effect.forkChild(Subscribable.get(sub));
    ...
    it("get fails with NoPropValue when source completes without emitting", async () => {
      const error = await scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.flip(Subscribable.get(sub));
        }),
      );
  ...
  describe("AC-10 stream failure on changes", () => {
    ...
    it("changes fails when the source fails after emitting", async () => {
      ...
      const exit = await scoped(
        Effect.gen(function* () {
          const sub = yield* Source.toSubscribable(source);
          return yield* pipe(Subscribable.changes(sub), Stream.runDrain, Effect.exit);
        }),
      );
    ...
    it("get still resolves to the emitted value despite the later failure", async () => {
      ...
      const value = await scoped(
        Effect.gen(function* () {
          ...
          yield* pipe(Subscribable.changes(sub), Stream.take(1), Stream.runDrain, Effect.ignore);
          return yield* Subscribable.get(sub);
        }),
      );
