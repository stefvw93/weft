...
any island observe writes made from any other island. Each root still has its
own lifetime: `handle.unmount()` tears down one island, `WeftApp.dispose(app)`
tears down everything (roots first, then the layer).
...
- `ControlsIsland` mutates the counter from `onclick` effect handlers; the
  handlers read `Counter` straight from the app layer — no props, no capture.
